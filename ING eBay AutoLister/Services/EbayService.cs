...
{
    public const string ProductionOAuthAcceptedUrl = "https://ingmining.com/oauth";
    public const string ProductionRuName = "Nicholas_Squire-Nicholas-AutoLi-xrbbwnfdq";
    ...
    public async Task<EbayOAuthRedirectExchangeResult> ExchangeProductionRedirectUrlAsync(string redirectUrl)
    {
        ...
        return new EbayOAuthRedirectExchangeResult(
            ...
            code, state ?? "", ProductionOAuthAcceptedUrl, ProductionRuName);
    }
    ...
    {
        var c = creds.Get();
        if (forceProduction || !c.EbaySandbox)
            return ProductionRuName;
    ...
    private static (string Code, string State) ParseProductionRedirectUrl(string redirectUrl)
    {
        if (!Uri.TryCreate(redirectUrl, UriKind.Absolute, out var uri))
            throw new InvalidOperationException("Paste the full eBay redirect URL from https://ingmining.com/oauth.");

        var isValidHost = uri.Scheme.Equals("https", StringComparison.OrdinalIgnoreCase) &&
                          uri.Host.Equals("ingmining.com", StringComparison.OrdinalIgnoreCase);
        if (!isValidHost)
            throw new InvalidOperationException($"Redirect URL must be from ingmining.com. Got: {uri.Host}");
