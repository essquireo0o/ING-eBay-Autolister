(() => {
  ...
  async function checkTrialStatus() {
    try {
      ...
      // Update trial badge in topbar
      const trialBadge = $('trial-badge');
      if (trialBadge) {
        if (!t.licensed && t.daysRemaining <= 7 && t.daysRemaining > 0) {
          trialBadge.textContent = t.daysRemaining === 1 ? '1 day left' : `${t.daysRemaining} days left`;
          trialBadge.classList.remove('hidden');
        } else if (!t.licensed && t.daysRemaining > 7) {
          trialBadge.textContent = `Trial: ${t.daysRemaining} days left`;
          trialBadge.classList.remove('hidden');
        } else {
      ...
      // Show paywall if expired and not licensed
      if (t.expired) showTrialPaywall();
    } catch { /* non-fatal */ }
  }
  ...
  function parseOAuthRedirectUrl(raw) {
    ...
    if (url.protocol !== 'https:' || url.hostname.toLowerCase() !== 'ingmining.com' || url.pathname.replace(/\/$/, '') !== '/oauth') {
      throw new Error('Redirect URL must start with https://ingmining.com/oauth.');
    }

    const code = url.searchParams.get('code') || '';
    const state = url.searchParams.get('state') || '';
    if (!code) throw new Error('Redirect URL is missing the code parameter.');
    return { code, state };
  }
  ...
  function createPhotoSlot(index) {
    ...
    slot.addEventListener('click', () => {
      if (!slot.classList.contains('has-image')) fileInput.click();
    });
